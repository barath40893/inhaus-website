    ...
    <section className="py-32 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div text-center mb-20>
          <div inline-block mb-4 px-6 py-2 glass rounded-full border border-orange-500/30 animate-fadeInUp>
            <span text-sm font-semibold gradient-text>Premium Features</span>
          </div>
          <h2 text-5xl md:text-6xl font-bold text-white mb-6 animate-fadeInUp\" style={{animationDelay: '0.1s'}}>
            Everything You Need for a <span gradient-text>Smart Home</span>
          </h2>
        ...
        <div grid md:grid-cols-3 gap-8>
          {features.map((feature, index) => (
            <div
              key={index}
              group premium-card glass-dark p-8 rounded-3xl hover:scale-105 transition-all duration-500 animate-fadeInUp\"
            ...
            >
              <div className={`w-16 h-16 bg-gradient-to-br ${feature.gradient} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 shadow-lg`}>
                <feature.icon text-white\" size={32} />
              </div>
              <h3 text-2xl font-bold text-white mb-4 group-hover:gradient-text transition-all duration-300>
                {feature.title}
              </h3>
              <p text-gray-400 leading-relaxed>{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
